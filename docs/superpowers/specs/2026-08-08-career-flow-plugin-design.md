...
- **Adopted from prior-art review** (career-ops, brag, resume-career-toolkit): outreach & follow-ups flow, reviewer pass on captured entries, raw-input provenance, ATS check on resumes, resume variant tracking (via a lightweight application log — deliberately not a full status-pipeline tracker), and GitHub-activity as a passive-capture source.
...
│   ├── outreach/SKILL.md        # hiring manager/recruiter identification, LinkedIn/email drafts (never sends), follow-up cadence
...
│   ├── applications/<slug>.md   # lightweight application log: company, role, resume variant used, contacts, follow-ups
...
---
id: proj-career-flow
type: project                 # experience | project | skill | education | publication | story | application
...
## Raw notes
...the user's original words from capture, preserved verbatim (provenance) —
later outputs can re-angle the same accomplishment from the source material...
```
...
6. **Reviewer pass (quality gate):** before any new/updated entry is finalized, a review step checks it for missing quantifiable metrics, vague impact statements, and absent dates, and prompts the user for the gaps. Entries enter the knowledge base resume-ready or explicitly flagged `needs-metrics`.
7. **Provenance:** capture flows preserve the user's original words verbatim in the entry's `## Raw notes` section alongside the refined narrative, so later outputs can re-angle an accomplishment from source material.
...
- **capture** — on-demand guided interview: "log a project" → asks impact, metrics, skills used, story-worthy moments; ends with the reviewer pass (memory-management item 6) before the entry is finalized.
...
- **passive** (opt-in, default off) — two config-selectable sources, both feeding `data/inbox.md` candidates: (a) a Claude Code SessionEnd/Stop hook that scans the session for career-noteworthy work; (b) a GitHub-activity scan (via `gh`) over merged PRs since the last scan — tool-independent, so it works regardless of which agent did the coding. `config.yaml` example: `passive: {enabled: false, sources: [github, claude_sessions]}`.

## Output Flows

- **resume** — input: job posting (text/URL). Selects relevant entries from INDEX + entity files, ranks by relevance to the posting, fills the Typst template, compiles to PDF in `outputs/resumes/<company>-<role>-<date>/`. Keeps the tailoring rationale alongside the PDF. Then: (a) **ATS check** — extracts text back out of the compiled PDF and verifies it at three layers (file parses cleanly, structure/section order survives, posting keywords present), fixing and recompiling if not; (b) **variant logging** — records which resume variant went to which employer in `data/applications/<slug>.md`.
...
- **outreach** — for an application: identify the likely hiring manager/recruiter (user-provided or web research), draft short LinkedIn messages and follow-up emails (drafts only — never sends anything), and record contacts and follow-up dates in the application log. A "due follow-ups" view lists applications whose next follow-up date has passed.
...
2. **Outputs** — **resume** (Typst template + compile + ATS check + variant logging), **cover-letter**, **interview-prep**, and **outreach** skills; application-log entity type.
3. **Maintenance cadence** — **checkin** and **maintain** skills; passive capture (opt-in): GitHub-activity scan + Claude Code session hook.
